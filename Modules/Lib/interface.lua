...
pcall(function()
    config = httpService:JSONDecode(readfile(FolderPath .. placeID .. ".json"));
end);

local function saveConfig()
    writefile(FolderPath .. placeID .. ".json", httpService:JSONEncode(config));
end;

local oldScript = script;
local popupGui = game:GetObjects("rbxassetid://8990177828")[1];
ParentGui(popupGui);
script = popupGui.mainScript;
local popup = loadstring(popupGui.mainScript.Source)();
script = oldScript;

local owlLibGui = game:GetObjects("rbxassetid://8990182637")[1];
ParentGui(owlLibGui);
